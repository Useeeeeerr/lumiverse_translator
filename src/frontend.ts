...
function request(ctx: SpindleFrontendContext, payload: Record<string, unknown>, timeoutMs = 120000): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    pending.set(requestId, (response) => {
      clearTimeout(timer);
      pending.delete(requestId);
      if (response?.type === "error") reject(new Error(response.error || "알 수 없는 오류"));
...
function setStatus(root: ParentNode, message: string, kind: "idle" | "ok" | "error" = "idle") {
...
function q<T extends Element>(root: ParentNode, selector: string): T {
  ...
  if (!el) throw new Error(`요소를 찾지 못했습니다: ${selector}`);
  return el;
}

function readSettings(root: ParentNode): Settings {
  return {
    enabled: q<HTMLInputElement>(root, "[name=enabled]").checked,
    includeChatHistory: q<HTMLInputElement>(root, "[name=includeChatHistory]").checked,
    historyCount: Number(q<HTMLInputElement>(root, "[name=historyCount]").value || DEFAULT_SETTINGS.historyCount),
    temperature: Number(q<HTMLInputElement>(root, "[name=temperature]").value || DEFAULT_SETTINGS.temperature),
    maxTokens: Number(q<HTMLInputElement>(root, "[name=maxTokens]").value || DEFAULT_SETTINGS.maxTokens),
    systemPrompt: q<HTMLTextAreaElement>(root, "[name=systemPrompt]").value,
    translatePrompt: q<HTMLTextAreaElement>(root, "[name=translatePrompt]").value,
    inputTranslatePrompt: q<HTMLTextAreaElement>(root, "[name=inputTranslatePrompt]").value,
    displayMode: q<HTMLSelectElement>(root, "[name=displayMode]").value as Settings["displayMode"],
  };
}

function renderSettings(root: HTMLElement, settings: Settings) {
  root.innerHTML = `
...
function openInputModal(ctx: SpindleFrontendContext) {
  ...
  modal.root.querySelector("[data-run]")?.addEventListener("click", async () => {
    try {
      ...
      const text = modal.root.querySelector<HTMLTextAreaElement>("[data-input]")?.value || "";
      const response = await request(ctx, { type: "translate_input", text });
      const output = modal.root.querySelector<HTMLTextAreaElement>("[data-output]");
      if (output) output.value = response.translation || "";
...
function openActionModal(ctx: SpindleFrontendContext) {
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  const unsubBackend = ctx.onBackendMessage((payload: any) => {
    if (payload?.requestId && pending.has(payload.requestId)) pending.get(payload.requestId)?.(payload);
  });
